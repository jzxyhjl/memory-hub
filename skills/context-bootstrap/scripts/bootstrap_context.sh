...
SESSION_CONTEXT_FILE="${SESSION_CONTEXT_FILE:-/Users/ggsk/.codex/SESSION_CONTEXT.md}"
MEMORY_API_BASE="${MEMORY_API_BASE:-http://127.0.0.1:8787}"
MEMORY_API_TOKEN="${MEMORY_API_TOKEN:-}"
QUERY="${1:-当前任务 上下文 决策 TODO}"
LIMIT="${2:-8}"
MEMORY_ENV_FILE="${MEMORY_ENV_FILE:-/Users/ggsk/memory-hub/.env}"

if [[ -z "${MEMORY_API_TOKEN}" && -f "${MEMORY_ENV_FILE}" ]]; then
  set -a
...
if [[ ! -f "${SESSION_CONTEXT_FILE}" ]]; then
  echo "[ERROR] SESSION_CONTEXT file not found: ${SESSION_CONTEXT_FILE}"
  exit 1
fi
